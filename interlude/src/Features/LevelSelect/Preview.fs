...
type Preview(info: LoadedChartInfo, change_rate: Rate -> unit) as this =
    ...
    let mutable playfield =
        Playfield(info.WithColors, playstate, Content.NoteskinConfig, false, info.WithMods.ModsSelected).With(LanecoverOverReceptors())
    ...
    let change_chart_listener =
        SelectedChart.on_chart_change_finished.Subscribe(fun info ->
            ...
            playfield <- Playfield(info.WithColors, playstate, Content.NoteskinConfig, false, info.WithMods.ModsSelected).With(LanecoverOverReceptors())
